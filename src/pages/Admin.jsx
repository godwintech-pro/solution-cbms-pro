  ...
  if (userRole !== 'Super Admin') {
    return (
      <div style={styles.accessDenied}>
        ...
        <p style={styles.accessSub}>This module is restricted to Super Admin only.</p>
      </div>
    );
  }
  ...
  async function handleSave() {
    ...
    try {
      await setDoc(doc(db, 'systemSettings', 'config'), {
        ...form, updatedBy: userName || 'Unknown', updatedAt: serverTimestamp(),
      });
      await addDoc(collection(db, 'auditLog'), {
        action: 'SETTINGS_UPDATED', description: 'Business settings updated',
        performedBy: userName || 'Unknown', createdAt: serverTimestamp(),
      });
      setSaved(true);
  ...
  return (
    <div>
      <div style={styles.sectionCard}>
        ...
        <p style={styles.sectionSub}>This information appears on printed reports and documents.</p>
        <div style={styles.formGrid}>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Company Name *</label>
            <input style={styles.input} name="companyName" value={form.companyName} onChange={handleChange} placeholder="e.g. Solution Enterprises" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Address</label>
            <input style={styles.input} name="companyAddress" value={form.companyAddress} onChange={handleChange} placeholder="e.g. Lusaka, Zambia" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Phone</label>
            <input style={styles.input} name="phone" value={form.phone} onChange={handleChange} placeholder="e.g. +260 977 000 000" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Email</label>
            <input style={styles.input} name="email" value={form.email} onChange={handleChange} placeholder="e.g. info@solution.co.zm" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Website</label>
            <input style={styles.input} name="website" value={form.website} onChange={handleChange} placeholder="e.g. www.solution.co.zm" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Tax / TPIN Number</label>
            <input style={styles.input} name="taxNumber" value={form.taxNumber} onChange={handleChange} placeholder="e.g. 1234567890" />
          </div>
        </div>
      </div>
      ...
      <div style={styles.sectionCard}>
        ...
        <p style={styles.sectionSub}>Controls how the system calculates alerts and displays data.</p>
        <div style={styles.formGrid}>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Currency Symbol</label>
            <input style={styles.input} name="currency" value={form.currency} onChange={handleChange} placeholder="e.g. K" />
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Financial Year Starts</label>
            <select style={styles.input} name="financialYearStart" value={form.financialYearStart} onChange={handleChange}>
              {months.map((m) => <option key={m.value} value={m.value}>{m.label}</option>)}
            </select>
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Expiry Warning Days</label>
            <input style={styles.input} name="expiryWarningDays" type="number" value={form.expiryWarningDays} onChange={handleChange} placeholder="Days before expiry to warn" />
            <p style={styles.inputHint}>Products expiring within this many days will be flagged.</p>
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Low Stock Threshold</label>
            <input style={styles.input} name="lowStockThreshold" type="number" value={form.lowStockThreshold} onChange={handleChange} placeholder="Min quantity before alert" />
            <p style={styles.inputHint}>Default reorder level for new products.</p>
          </div>
          <div style={styles.inputGroup}>
            <label style={styles.label}>Timezone</label>
            <select style={styles.input} name="timezone" value={form.timezone} onChange={handleChange}>
        ...
        <div style={{ textAlign: 'right', marginTop: '20px' }}>
          {saved && <span style={styles.savedBadge}>✅ Settings saved successfully!</span>}
  ...
  return (
    <div>
      <div style={styles.sectionCard}>
        <div style={styles.sectionHeader}>
          <div>
            ...
            <p style={styles.sectionSub}>Download all data as a single JSON file. Store safely on your computer or cloud drive.</p>
          </div>
          {lastExport && <span style={styles.lastExportBadge}>Last export: {lastExport}</span>}
        </div>
          ...
          <div style={styles.backupMainLeft}>
            ...
            <div>
              ...
              <p style={styles.backupMainDesc}>
                Exports ALL collections including products, stock, reports, orders, salaries,
                cash handovers, users, branches, notifications and audit log.
                Download this weekly to protect your business data.
              </p>
            </div>
          </div>
          <button style={exporting === 'all' ? styles.saveBtnDisabled : styles.backupMainBtn}
      ...
      <div style={styles.sectionCard}>
        ...
        <p style={styles.sectionSub}>Download specific data as CSV files that open directly in Excel.</p>
        <div style={styles.exportGrid}>
          {exports.map((exp) => (
            <div key={exp.id} style={styles.exportCard}>
              ...
              <button style={exporting === exp.id ? styles.exportBtnDisabled : styles.exportBtn}
                onClick={() => exportCollection(exp.id)} disabled={exporting === exp.id}>
...
function AuditLog() {
  ...
  const actionTypes = [
    'SETTINGS_UPDATED', 'DATA_EXPORTED', 'SOFT_RESET', 'HARD_RESET',
    'NOTIFICATIONS_CLEARED', 'USER_CREATED', 'USER_UPDATED', 'USER_DELETED',
    'SALARY_ADDED', 'CASH_HANDOVER', 'CASH_CONFIRMED', 'STOCK_ADJUSTED',
    'STOCK_TAKE', 'ORDER_APPROVED', 'ORDER_REJECTED', 'REPORT_SUBMITTED', 'INVESTMENT_ADDED',
  ];
    ...
    <div style={styles.sectionCard}>
      <div style={styles.sectionHeader}>
        <div>
          ...
          <p style={styles.sectionSub}>Complete record of all system actions. {logs.length} events logged.</p>
        </div>
      </div>
      <div style={styles.filterRow}>
        ...
        {(filterUser || filterType) && (
          <button style={styles.clearBtn} onClick={() => { setFilterUser(''); setFilterType(''); }}>✕ Clear</button>
        )}
      </div>
      {filtered.length === 0 ? (
        <div style={styles.empty}>No audit log entries yet. Actions will be recorded here as the system is used.</div>
      ) : (
            ...
            return (
              <div key={log.id} style={styles.auditItem}>
                <div style={styles.auditLeft}>
                  <span style={{ ...styles.auditBadge, background: typeStyle.bg, color: typeStyle.color }}>
...
function DataManagement() {
  ...
  useEffect(() => {
    async function loadStats() {
      const collections = ['notifications', 'auditLog', 'broadcasts', 'stockAdjustments', 'stockTakes', 'dailyReports', 'orders'];
      const results = {};
      for (const col of collections) {
        try {
          const snap = await getDocs(collection(db, col));
  ...
  async function clearNotifications(daysOld) {
    ...
    try {
      ...
      snap.docs.forEach((d) => {
        const createdAt = d.data().createdAt?.seconds ? new Date(d.data().createdAt.seconds * 1000) : null;
        if (createdAt && createdAt < cutoff) { batch.delete(d.ref); count++; }
      });
      await batch.commit();
      await addDoc(collection(db, 'auditLog'), {
        action: 'NOTIFICATIONS_CLEARED', description: `Cleared ${count} notifications older than ${daysOld} days`,
        performedBy: userName || 'Unknown', createdAt: serverTimestamp(),
      });
  ...
  async function clearReadNotifications() {
    ...
    try {
      ...
      const batch = writeBatch(db);
      let count = 0;
      snap.docs.forEach((d) => { if (d.data().read === true) { batch.delete(d.ref); count++; } });
      await batch.commit();
      await addDoc(collection(db, 'auditLog'), {
        action: 'NOTIFICATIONS_CLEARED', description: `Cleared ${count} read notifications`,
        performedBy: userName || 'Unknown', createdAt: serverTimestamp(),
      });
  ...
  return (
    <div>
      ...
      <div style={styles.sectionCard}>
        ...
        <div style={styles.actionGrid}>
          <div style={styles.actionCard}>
            ...
            <button style={loading === 'read_notif' ? styles.actionBtnDisabled : styles.actionBtn}
          ...
          <div style={styles.actionCard}>
            ...
            <button style={loading === 'notif_30' ? styles.actionBtnDisabled : styles.actionBtn}
          ...
          <div style={styles.actionCard}>
            ...
            <button style={loading === 'notif_60' ? styles.actionBtnDisabled : styles.actionBtn}
          ...
          <div style={styles.actionCard}>
            ...
            <button style={loading === 'notif_90' ? styles.actionBtnDisabled : styles.actionBtn}
      ...
      <div style={styles.sectionCard}>
        ...
        <button style={loading === 'audit' ? styles.actionBtnDisabled : styles.actionBtn}
...
function DangerZone() {
  ...
  // 'users' and 'systemSettings' are ALWAYS preserved
  // regardless of reset type to prevent lockout
  ...
  async function handleSoftReset() {
    ...
    try {
      const collectionsToDelete = [
        ...
        'broadcasts', 'investments', 'salaries', 'purchaseOrders',
      ];
      let totalDeleted = 0;
      ...
      // Reset stock quantities to 0 but keep entries
      const stockSnap = await getDocs(collection(db, 'stock'));
      const stockBatch = writeBatch(db);
      stockSnap.docs.forEach((d) => stockBatch.update(d.ref, { currentQuantity: 0 }));
      await stockBatch.commit();

      await addDoc(collection(db, 'auditLog'), {
        action: 'SOFT_RESET',
        description: `Soft reset performed. ${totalDeleted} records cleared. Stock quantities reset to 0. Products, branches, users and system settings preserved.`,
        performedBy: userName || 'Unknown',
        createdAt: serverTimestamp(),
      });
      setShowSoftModal(false);
      alert(`✅ Soft reset complete. ${totalDeleted} records cleared.\n\nProtected: Users, system settings, products, branches, categories, suppliers.`);
    } catch (err) { alert(err.message); }
    setSoftLoading(false);
  }

  async function handleHardReset() {
    if (hardConfirmText !== 'DELETE ALL DATA') return alert('Type DELETE ALL DATA exactly to confirm.');
    setHardLoading(true);
    try {
      // ⚠️ PROTECTED: 'users' and 'systemSettings' are NEVER included
      const allCollections = [
        ...
        'suppliers', 'branches', 'branchChats', 'auditLog',
      ];
      ...
      // Log the reset — auditLog was cleared above so re-add entry
      await addDoc(collection(db, 'auditLog'), {
        action: 'HARD_RESET',
        description: 'Full system reset performed. All data deleted except users and system settings.',
        performedBy: userName || 'Unknown',
        createdAt: serverTimestamp(),
      });
      setShowHardModal(false);
      setHardConfirmText('');
      alert('⚠️ Full system reset complete.\n\nAll data deleted.\n🔒 Protected: Your user account and system settings were preserved.\n\nPlease refresh the page.');
    } catch (err) { alert(err.message); }
  ...
  return (
    <div>
      {/* Protection Notice */}
      <div style={styles.protectionBanner}>
        <span style={{ fontSize: '24px' }}>🔒</span>
        <div>
          <p style={styles.protectionTitle}>Always Protected — Never Deleted</p>
          <p style={styles.protectionDesc}>
            <strong>User accounts</strong> and <strong>system settings</strong> are permanently protected
            from all resets. You will never be locked out of the system.
          </p>
        </div>
      </div>
      ...
      <div style={{ ...styles.sectionCard, border: '1px solid #ffe082' }}>
        <div style={styles.dangerCardHeader}>
          <div>
            <h3 style={{ ...styles.sectionTitle, color: '#f39c12' }}>🔄 Soft Reset — Clear Test Data</h3>
            <p style={styles.sectionSub}>Clears all operational data but keeps your setup intact. Use this to clear test data before going live.</p>
          </div>
          <button style={styles.softResetBtn} onClick={() => setShowSoftModal(true)}>🔄 Soft Reset</button>
        </div>
        <div style={styles.dangerList}>
          ...
          <div style={styles.dangerCol}>
            ...
            {['Products & Categories', 'Suppliers', 'Branches', 'Audit Log'].map((item) => (
              <p key={item} style={styles.dangerKeepItem}>✅ {item}</p>
            ))}
            <p style={{ ...styles.dangerKeepItem, fontWeight: '800' }}>🔒 Users & Roles (always protected)</p>
            <p style={{ ...styles.dangerKeepItem, fontWeight: '800' }}>🔒 System Settings (always protected)</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...styles.sectionCard, border: '2px solid #dc3545' }}>
        <div style={styles.dangerCardHeader}>
          <div>
            <h3 style={{ ...styles.sectionTitle, color: '#dc3545' }}>☢️ Hard Reset — Full System Wipe</h3>
            <p style={styles.sectionSub}>
              Permanently deletes all business data. The system will be empty but you can still log in.
              This cannot be undone.
            </p>
          </div>
          <button style={styles.hardResetBtn} onClick={() => setShowHardModal(true)}>☢️ Full Reset</button>
        </div>
        <div style={styles.hardResetWarning}>
          🚨 This will delete: All products, all stock, all reports, all branches, all orders,
          all financial records, all broadcasts and audit log.
          <br /><br />
          🔒 <strong>Always preserved:</strong> Your user account and system settings — you will never be locked out.
        </div>
      </div>
      ...
      {showSoftModal && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <p style={styles.modalWarningDesc}>
              🔒 <strong>Protected — will NOT be deleted:</strong> Users, system settings,
              products, categories, suppliers and branches.
            </p>
            <p style={styles.modalWarningDesc}><strong>Have you downloaded a backup first?</strong></p>
            <div style={styles.modalActions}>
              <button style={styles.cancelBtn} onClick={() => setShowSoftModal(false)}>Cancel</button>
              <button style={softLoading ? styles.saveBtnDisabled : styles.softResetBtn}
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            <div style={{ ...styles.modalWarningHeader, background: '#fff0f0' }}>
              ...
              <h3 style={{ ...styles.modalWarningTitle, color: '#dc3545' }}>FULL SYSTEM RESET</h3>
            </div>
            <p style={styles.modalWarningDesc}>
              🚨 <strong>WARNING:</strong> This will permanently delete all business data.
              Products, stock, reports, orders, branches and financial records will be gone.
            </p>
            <p style={{ ...styles.modalWarningDesc, color: '#28a745' }}>
              🔒 <strong>You will NOT be locked out.</strong> Your user account and system
              settings are always preserved regardless of reset type.
            </p>
            <p style={styles.modalWarningDesc}>
            ...
            <div style={styles.modalActions}>
              ...
              <button
                style={hardConfirmText !== 'DELETE ALL DATA' || hardLoading ? styles.saveBtnDisabled : styles.hardResetBtn}
                onClick={handleHardReset}
                disabled={hardConfirmText !== 'DELETE ALL DATA' || hardLoading}>
                {hardLoading ? '⏳ Deleting...' : '☢️ DELETE EVERYTHING'}
              </button>
            </div>
          </div>
        </div>
...
const styles = {
  ...
  protectionBanner: { background: '#e6f9ee', border: '1px solid #b2dfdb', borderRadius: '12px', padding: '16px 20px', marginBottom: '16px', display: 'flex', gap: '16px', alignItems: 'flex-start' },
  protectionTitle: { fontSize: '14px', fontWeight: '800', color: '#28a745', margin: '0 0 4px' },
  protectionDesc: { fontSize: '13px', color: '#555', margin: 0 },
