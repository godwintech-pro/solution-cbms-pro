...
function ReportsOverview({ userRole }) {
  ...
  const totalExpenses = todayReports.reduce(
    ...
    <div>
      ...
      <div style={styles.statsGrid}>
        ...
        <div
          style={{
            ...styles.statCard,
            borderTop: `4px solid ${totalVariance < 0 ? '#dc3545' : '#28a745'}`,
          }}
        >
                ...
                >
                  ...
                  {report ? (
                    ...
                    <p style={styles.notSubmittedText}>
                      Report not yet submitted for today.
                    </p>
                  )}
                </div>
              );
...
function SubmitReport({ userName, userRole, userBranch, onSubmitted }) {
  ...
  // Check if report already submitted for selected branch today
  ...
  function addExpense() {
    setForm({
      ...form,
      expenses: [...form.expenses, { description: '', amount: '' }],
    });
  }
  ...
  async function handleSubmit() {
    ...
    if (alreadySubmitted)
      return alert('Report already submitted for this branch today.');
    ...
    try {
      const reportData = {
        ...
        expectedCash,
        variance,
        stockReceived: form.stockReceived,
        stockSupplier: form.stockSupplier,
        stockNotes: form.stockNotes,
      ...
      if (variance < 0) {
        await addDoc(collection(db, 'notifications'), {
          type: 'VARIANCE_ALERT',
          message: `⚠️ Cash variance of K ${Math.abs(variance).toFixed(
            2
          )} detected at ${reportData.branchName}`,
          branchId: form.branchId,
          date: form.date,
          read: false,
          createdAt: serverTimestamp(),
    ...
    return (
      <div style={styles.successBox}>
        ...
        <p style={styles.successSub}>
          Mr. Mwanza has been notified. Redirecting...
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={styles.sectionCard}>
      ...
      <p style={styles.sectionSub}>
        Fill in all sections carefully. Report cannot be edited after
        submission.
      </p>
      ...
      <div style={styles.formSection}>
        ...
        {alreadySubmitted && (
          <div style={styles.warningBox}>
            ⚠️ A report has already been submitted for this branch today.
          </div>
        )}
      </div>
      ...
      <div style={styles.formSection}>
        <div style={styles.formSectionHeader}>
          ...
          <button style={styles.addExpenseBtn} onClick={addExpense}>
            + Add Expense
          </button>
        </div>
        {form.expenses.map((expense, index) => (
          <div key={index} style={styles.expenseRow}>
            <input
              ...
              placeholder="Expense description e.g. Electricity"
        ...
        <div style={styles.expenseTotal}>
          Total Expenses: <strong>K {totalExpenses.toFixed(2)}</strong>
        </div>
      </div>
            ...
            <>
              <div style={styles.inputGroup}>
                ...
                <input
                  style={styles.input}
                  name="stockSupplier"
                  value={form.stockSupplier}
                  placeholder="e.g. Baxter Zambia"
                  onChange={handleChange}
                />
              </div>
              <div style={{ ...styles.inputGroup, gridColumn: '1 / -1' }}>
                ...
                <input
                  ...
                  placeholder="e.g. Paracetamol x200, Amoxicillin x100"
                  onChange={handleChange}
                />
              </div>
            </>
      ...
      <div style={styles.summaryBox}>
        ...
        <div style={styles.summaryGrid}>
          <div style={styles.summaryRow}>
            <span style={styles.summaryLabel}>Opening Float</span>
            <span style={styles.summaryValue}>K {openingFloat.toFixed(2)}</span>
          </div>
          <div style={styles.summaryRow}>
            <span style={styles.summaryLabel}>Total Sales</span>
            <span style={styles.summaryValue}>K {totalSales.toFixed(2)}</span>
          </div>
          <div style={styles.summaryRow}>
            <span style={styles.summaryLabel}>Less Expenses</span>
          ...
          >
            ...
            <span style={{ ...styles.summaryValue, fontWeight: '700' }}>
              K {expectedCash.toFixed(2)}
            </span>
          </div>
          <div style={styles.summaryRow}>
            <span style={styles.summaryLabel}>Actual Cash</span>
            <span style={styles.summaryValue}>K {actualCash.toFixed(2)}</span>
          </div>
          <div
            style={{
              ...styles.summaryRow,
              background: variance < 0 ? '#fff0f0' : '#e6f9ee',
              borderRadius: '8px',
              padding: '10px',
              marginTop: '4px',
            }}
      ...
      <div style={styles.formSection}>
        ...
        <textarea
          ...
          placeholder="Any other observations or notes for management..."
          onChange={handleChange}
          rows={3}
        />
      </div>
...
function AllReports({ userRole, userBranch }) {
  ...
  function handlePrint(report) {
    ...
    win.document.write(generateReportPrint(report));
    win.document.close();
    win.print();
  }

  function generateReportPrint(r) {
    const expenses = (r.expenses || [])
      .map(
        (e) =>
          `<tr><td>${e.description}</td><td>K ${parseFloat(e.amount).toFixed(
            2
          )}</td></tr>`
      )
      .join('');
    ...
    return `
      ...
      <body>
        ...
        <div class="section">
          <h3>💰 Cash Section</h3>
          <table>
            <tr><td>Opening Float</td><td>K ${(r.openingFloat || 0).toFixed(
              2
            )}</td></tr>
            <tr><td>Total Sales</td><td>K ${(r.totalSales || 0).toFixed(
              2
            )}</td></tr>
            <tr><td>Total Expenses</td><td>K ${(r.totalExpenses || 0).toFixed(
              2
            )}</td></tr>
            <tr><td><strong>Expected Cash</strong></td><td><strong>K ${(
              r.expectedCash || 0
            ).toFixed(2)}</strong></td></tr>
            <tr><td>Actual Cash</td><td>K ${(r.actualCash || 0).toFixed(
              2
            )}</td></tr>
          </table>
          <div class="variance ${r.variance < 0 ? 'deficit' : 'balanced'}">
            Variance: K ${(r.variance || 0).toFixed(2)} ${
      r.variance < 0 ? '⚠️ Deficit' : '✅ Balanced'
    }
          </div>
        </div>
            ...
            <div
              ...
              style={{
                ...
                borderLeft: `4px solid ${
                  report.variance < 0 ? '#dc3545' : '#28a745'
                }`,
              }}
            >
              <div style={styles.reportCardHeader}>
                ...
                <div style={styles.reportCardActions}>
                  <span
                    style={{
                      ...styles.badge,
                      background: report.variance < 0 ? '#fff0f0' : '#e6f9ee',
                      color: report.variance < 0 ? '#dc3545' : '#28a745',
                    }}
                  >
              ...
              <div style={styles.reportQuickStats}>
                <div style={styles.reportStat}>
                  <p style={styles.reportStatLabel}>Sales</p>
                  <p style={styles.reportStatValue}>
                    K {(report.totalSales || 0).toFixed(2)}
                  </p>
                </div>
                <div style={styles.reportStat}>
                  <p style={styles.reportStatLabel}>Expenses</p>
                  <p style={styles.reportStatValue}>
                    K {(report.totalExpenses || 0).toFixed(2)}
                  </p>
                </div>
                ...
                <div style={styles.reportStat}>
                  <p style={styles.reportStatLabel}>Cash in Hand</p>
                  <p style={styles.reportStatValue}>
                ...
                <div style={styles.reportDetail}>
                  ...
                  <div style={styles.detailSection}>
                    ...
                    <div style={styles.detailRow}>
                      ...
                      <span>{report.stockReceived}</span>
                    </div>
                    {report.stockReceived === 'Yes' && (
                      <>
                        <div style={styles.detailRow}>
                          <span>Supplier</span>
                          <span>{report.stockSupplier || '—'}</span>
                        </div>
                        <div style={styles.detailRow}>
                          <span>Items</span>
                          <span>{report.stockNotes || '—'}</span>
                        </div>
                      </>
                    )}
                    <div style={styles.detailRow}>
                      <span>Damages/Losses</span>
                      <span>{report.damages}</span>
                    </div>
                    {report.damages === 'Yes' && (
                      <div style={styles.detailRow}>
                        <span>Details</span>
                        <span>{report.damageNotes || '—'}</span>
                      </div>
                    )}
                  </div>
