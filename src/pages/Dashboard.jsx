    ...
    <div>
      <div style={ovStyles.statsGrid}>
        <div style={{ ...ovStyles.statCard, borderTop: '4px solid #0f3460' }}>
          ...
          <p style={ovStyles.statSub}>{todayReports.length} branch{todayReports.length !== 1 ? 'es' : ''} reported</p>
        </div>
        <div style={{ ...ovStyles.statCard, borderTop: '4px solid #28a745' }}>
          <p style={ovStyles.statLabel}>📋 Total Expenses Today</p>
          <p style={{ ...ovStyles.statValue, color: '#e94560' }}>K {totalExpensesToday.toFixed(2)}</p>
          <p style={ovStyles.statSub}>Profit: K {(totalSalesToday - totalExpensesToday).toFixed(2)}</p>
        </div>
        <div style={{ ...ovStyles.statCard, borderTop: '4px solid #e94560' }}>
          <p style={ovStyles.statLabel}>⚠️ Expiring Products</p>
          <p style={{ ...ovStyles.statValue, color: expiringProducts.length > 0 ? '#f39c12' : '#28a745' }}>
        ...
        <div style={{ ...ovStyles.statCard, borderTop: '4px solid #f39c12' }}>
          ...
          <p style={{ ...ovStyles.statValue, color: pendingOrders.length > 0 ? '#f39c12' : '#28a745' }}>
      ...
      {totalVarianceToday < 0 && (
        <div style={ovStyles.alertBox}>
          ⚠️ Cash variance of <strong>K {Math.abs(totalVarianceToday).toFixed(2)}</strong> detected today. Investigate immediately.
        </div>
      )}
          ...
          {branches.length === 0 ? (
            ...
            <div style={ovStyles.branchList}>
              {branches.map((branch) => {
                ...
                return (
                  <div key={branch.id} style={{ ...ovStyles.branchItem, borderLeft: `4px solid ${report ? '#28a745' : '#dc3545'}` }}>
                    <div style={ovStyles.branchTop}>
                      <span style={ovStyles.branchName}>{branch.name}</span>
                      <span style={{ ...ovStyles.branchBadge, background: report ? '#e6f9ee' : '#fff0f0', color: report ? '#28a745' : '#dc3545' }}>
                    ...
                    {report ? (
                      <div style={ovStyles.branchStats}>
                        <div style={ovStyles.branchStat}>
                          ...
                          <p style={ovStyles.branchStatValue}>K {(report.totalSales || 0).toFixed(2)}</p>
                        </div>
                        <div style={ovStyles.branchStat}>
                          <p style={ovStyles.branchStatLabel}>Expenses</p>
                          <p style={ovStyles.branchStatValue}>K {(report.totalExpenses || 0).toFixed(2)}</p>
                        </div>
                        <div style={ovStyles.branchStat}>
                          <p style={ovStyles.branchStatLabel}>Cash</p>
                          <p style={ovStyles.branchStatValue}>K {(report.actualCash || 0).toFixed(2)}</p>
                        </div>
                        <div style={ovStyles.branchStat}>
                          <p style={ovStyles.branchStatLabel}>Variance</p>
                          <p style={{ ...ovStyles.branchStatValue, color: (report.variance || 0) < 0 ? '#dc3545' : '#28a745', fontWeight: '800' }}>
                            K {(report.variance || 0).toFixed(2)}{(report.variance || 0) < 0 && ' ⚠️'}
                          </p>
                        </div>
                      </div>
                    ) : (
                      <p style={ovStyles.notReported}>Report not submitted yet today.</p>
                    )}
                  </div>
                );
              })}
              ...
              <div style={ovStyles.notifList}>
                {notifications.slice(0, 6).map((n) => (
                  <div key={n.id} style={{
                    ...
                    borderLeft: `3px solid ${n.type === 'VARIANCE_ALERT' ? '#dc3545' : n.type === 'STOCK_ALERT' ? '#f39c12' : n.type === 'ORDER_SUBMITTED' ? '#0f3460' : '#28a745'}`,
                  }}>
              ...
              <div style={ovStyles.activityList}>
                {recentActivity.map((a, i) => (
                  <div key={i} style={ovStyles.activityItem}>
                    <div style={{ ...ovStyles.activityDot, background: a.color }}>{a.icon}</div>
      ...
      <div style={ovStyles.quickStatsRow}>
        <div style={ovStyles.quickStat}><p style={ovStyles.quickStatNum}>{branches.length}</p><p style={ovStyles.quickStatLabel}>Total Branches</p></div>
        <div style={ovStyles.quickStat}><p style={ovStyles.quickStatNum}>{products.length}</p><p style={ovStyles.quickStatLabel}>Products</p></div>
        <div style={ovStyles.quickStat}><p style={ovStyles.quickStatNum}>{stock.length}</p><p style={ovStyles.quickStatLabel}>Stock Entries</p></div>
        <div style={ovStyles.quickStat}><p style={ovStyles.quickStatNum}>{orders.length}</p><p style={ovStyles.quickStatLabel}>Total Orders</p></div>
        <div style={ovStyles.quickStat}><p style={ovStyles.quickStatNum}>{reports.length}</p><p style={ovStyles.quickStatLabel}>Total Reports</p></div>
        <div style={ovStyles.quickStat}>
          <p style={{ ...ovStyles.quickStatNum, color: expiringProducts.length > 0 ? '#f39c12' : '#28a745' }}>{expiringProducts.length}</p>
...
const ovStyles = {
  ...
  card: { background: 'white', borderRadius: '12px', padding: '20px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' },
  ...
  branchItem: { padding: '12px', background: '#fafafa', borderRadius: '8px', border: '1px solid #f0f0f0' },
  branchTop: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' },
  branchName: { fontSize: '14px', fontWeight: '700', color: '#1a1a2e' },
  branchBadge: { padding: '3px 10px', borderRadius: '12px', fontSize: '11px', fontWeight: '700' },
  ...
  notifItem: { padding: '10px 12px', borderRadius: '8px', border: '1px solid #f0f0f0' },
  ...
  activityDot: { width: '32px', height: '32px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '14px', flexShrink: 0, color: 'white' },
  activityInfo: { flex: 1 },
  activityMessage: { fontSize: '13px', fontWeight: '600', color: '#1a1a2e', margin: 0 },
  activitySub: { fontSize: '11px', color: '#aaa', margin: '2px 0 0' },
  quickStatsRow: { display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '12px' },
  quickStat: { background: 'white', borderRadius: '10px', padding: '16px', textAlign: 'center', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' },
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      (snap) => setNotifCount(snap.docs.filter((d) => !d.data().read).length)
    );
    return unsub;
  }, []);
    ...
    <div style={styles.container}>
      ...
      <div style={{ ...styles.main, marginLeft: sidebarWidth }}>
        <div style={styles.topBar}>
          ...
          <div style={{ ...styles.alertBadge, background: notifCount > 0 ? '#e94560' : '#28a745' }}>
...
const styles = {
  container: { display: 'flex', minHeight: '100vh', fontFamily: "'Segoe UI', sans-serif", background: '#f0f2f5' },
  sidebar: { background: 'linear-gradient(180deg, #1a1a2e 0%, #16213e 100%)', display: 'flex', flexDirection: 'column', padding: '10px 0', position: 'fixed', height: '100vh', overflowY: 'auto', overflowX: 'hidden', transition: 'width 0.3s ease', zIndex: 100 },
  toggleBtn: { alignSelf: 'flex-end', marginRight: '8px', marginBottom: '8px', background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', borderRadius: '6px', padding: '6px 10px', cursor: 'pointer', fontSize: '12px' },
  sidebarHeader: { display: 'flex', alignItems: 'center', gap: '10px', padding: '0 20px 20px', borderBottom: '1px solid rgba(255,255,255,0.1)' },
  ...
  alertBadge: { color: 'white', padding: '8px 16px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', transition: 'background 0.3s' },
  comingSoon: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px', background: 'white', borderRadius: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' },
